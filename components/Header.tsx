...
function Header() {
  ...
  const searchSong = (value:string) =>{
    for(let i=0; i<songsData.length; i++){
      if(value === songsData[i].name){
  ...
  return (
    <div className='flex flex-col'>
        <div className='w-full h-20  bg-black flex items-center justify-between'>
            ...
            <input type="text" className='bg-black border-2 border-gray-700 rounded-3xl mr-100 h-13 hover:duration-400 hover hover:border-2  hover:border-red-950 text-white text-2xl p-1.5 ml-4'  placeholder='Search your music' onChange={(e)=>searchSong(e.target.value)}/>
            </div>
        </div>
        <div>
